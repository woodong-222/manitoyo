...
function AuthPage() {
  ...
  const [isRematchReadyOpen, setIsRematchReadyOpen] = useState(false);
  ...
  return (
    <section className="auth-page">
      ...
      {isRematchReadyOpen ? (
        <div className="auth-page__modal">
          <div className="auth-page__modal-card auth-page__modal-card--blur auth-page__modal-card--small">
            <h3 className="auth-page__modal-title">새 링크 생성</h3>
            <p className="auth-page__modal-subtitle">
              새 링크가 생성되었습니다. 이전 링크는 그대로 유지됩니다.
            </p>
            <button
              onClick={() => {
                setIsRematchReadyOpen(false);
                setIsShareModalOpen(true);
              }}
              className="auth-page__modal-cta"
            >
